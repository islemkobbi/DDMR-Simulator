...
class robot_controller:
    def __init__(self):
        rospy.init_node('r2_robot_controller')

        self.Xr = [0, 0, 0, 0, 0]
        self.X = self.Xr 
        self.Xp = self.Xr
        
        self.cmdvel = rospy.Publisher("/robot2/cmd_vel", Twist, queue_size=10)

        rospy.Subscriber('/robot2/odom', Odometry, self.odom_callback)
        rospy.Subscriber('/robot2/estimated_state', State, self.est_state_callback)
        rospy.Subscriber('/robot2/ref_state', State, self.get_ref_callback)
