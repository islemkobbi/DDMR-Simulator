...
from ddmr_ma_controller.msg import Yout, State, APpose
...
class sensors_output:
    def __init__(self):
        rospy.init_node('r1_sensors_output')
        ...
        self.sens_out = rospy.Publisher("/robot1/sens_out", Yout, queue_size=10)
        self.ap_pose_out = rospy.Publisher("/robot1/ap_pose", APpose, queue_size=10)
    ...
    def cal_real_des(self):
        ...
        AP5 = [self.XY2r[0], self.XY2r[1]]
        AP6 = [self.XY3r[0], self.XY3r[1]]

        ed1 = self.get_errors(X[0], X[1])

        D1 = (self.AP1[0] - X[0])**2 + (self.AP1[1] - X[1])**2 + ed1[0]
        D2 = (self.AP2[0] - X[0])**2 + (self.AP2[1] - X[1])**2 + ed1[1]
        D3 = (self.AP3[0] - X[0])**2 + (self.AP3[1] - X[1])**2 + ed1[2]
        D4 = (self.AP4[0] - X[0])**2 + (self.AP4[1] - X[1])**2 + ed1[3]
    ...
    def pub_out(self, x):
        ...
        C = np.array([self.AP1[:2], self.AP2[:2], self.AP3[:2], self.AP4[:2], self.XY2p[:2], self.XY3p[:2]],dtype=float)
        msg1 = APpose()
