...
class robot_controller:
    def __init__(self):
        rospy.init_node('r3_robot_controller')

        self.Xr = [0, -1, 0, 0, 0]
        self.X = self.Xr 
        self.Xp = self.Xr
        
        self.cmdvel = rospy.Publisher("/robot3/cmd_vel", Twist, queue_size=10)

        rospy.Subscriber('/robot3/odom', Odometry, self.odom_callback)
        rospy.Subscriber('/robot3/estimated_state', State, self.est_state_callback)
        rospy.Subscriber('/robot3/ref_state', State, self.get_ref_callback)
